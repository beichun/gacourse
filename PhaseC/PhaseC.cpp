...
////////////////////// evolve ////////////////////////////////////////////////////
    int num_frames = 500;
    int skip_frames = 32;
    int num_evaluations = 2048/4;
    int population_size = 64*2/4;
...
//#pragma omp parallel for
    for (int i = 0; i < population_size; i++) {
        ...
        ArrayX3dRowMajor massPosition_i = initMassPosition();
...
////    beam search
    for (int generation_i = 0; generation_i<num_generation;generation_i++) {
//    down select
        Eigen::ArrayXi order = orderFitness(best_fitness_arr);
        ArrayXXdRowMajor springCoefa_arr_copy = springCoefa_arr;
        ArrayXXdRowMajor springCoefb_arr_copy = springCoefb_arr;
        Eigen::ArrayXd best_fitness_arr_copy = best_fitness_arr;

//#pragma omp parallel for
        for (int i = 0; i < selection_pressure; i++) {
            springCoefa_arr.col(i) = springCoefa_arr_copy.col(order(i));
            springCoefb_arr.col(i) = springCoefb_arr_copy.col(order(i));
            best_fitness_arr(i) = springCoefa_arr_copy(order(i));
            std::cout<<i<<" "<<best_fitness_arr(i)<<std::endl;
        }
//        std::cout<<"best fitness1 = "<<best_fitness_arr(0);
//        std::cout<<"best fitness2 = "<<best_fitness_arr(selection_pressure);
//        std::cout<<"best fitness3 = "<<best_fitness_arr(order(population_size))<<std::endl;
...
#pragma omp parallel for
        for (int i = 0; i < selection_pressure; i++) {
            ...
            springCoefa_arr.row(selection_pressure + i) = springCoefa_i;
            springCoefb_arr.row(selection_pressure + i) = springCoefb_i;
            ...
            best_fitness_arr(selection_pressure + i) = simulate(
                    num_frames,
                    skip_frames,
                    springCoefa_i,
                    springCoefb_i,
...
////    the best robot
    ArrayXdRowMajor springCoefa_i = springCoefa_arr.block(order(0), 0, 1, num_cubes);
    ArrayXdRowMajor springCoefb_i = springCoefb_arr.block(order(0), 0, 1, num_cubes);
